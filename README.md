...
body.classList.toggle('dark-theme', isDarkTheme); - This line toggles the dark-theme class of the body element. The classList property is a read-only list that contains the classes of an element. TIn this case, it adds the he toggle() method adds a class to the element if it does not have it, or removes it if it does. dark-theme class if isDarkTheme is true, and removes it if isDarkTheme is false.
